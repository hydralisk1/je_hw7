...
class Page:
    ...
    def verify_list_has_value(self, text: str, *locator):
        results = self.find_elements(*locator)
        results_contain_text = [p.text for p in results if \
                                all(item in p.text.lower().split() for item in text.lower().split())]
        assert len(results_contain_text) > 0, f"There's no result containing {text}."
